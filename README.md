...
**A `200` is not the same thing as a success here.** The server reports a failed _and_ a cancelled request as `COMPLETED` carrying an `error_type`, so `get()` rejects with the matching typed exception from [`routerErrors`](#router-errors-comfymodelsrun) rather than handing the failure back as a result. `events()` deliberately does not reject — it is a view of the queue's progress, and `get()` is the one that collects.
...
`timeoutMs` is a **client-side** bound with no server-side meaning — the queue's own timeouts are the server's. It covers the whole call: the submit, every poll, every retry of one, the pauses between them, and the result fetch. When it runs out — or when `signal` aborts — `subscribe` makes one best-effort `cancel()`, so a caller who has stopped waiting is not also still paying for a generation nobody will collect, and then rejects. Best-effort is literal: a cancel that itself fails is swallowed, because the timeout is the failure worth reporting and a masked one sends you looking in the wrong place. Use `submit` when the request should outlive the caller's patience.
