...
The result route's `200` declares the same two branches as the run route's, and `get()` (and `subscribe`) branch on its `Content-Type` exactly as `run` does: a model whose partner answers with its own media type resolves to the [`kind: "binary"`](#binary-results--a-model-that-returns-audio-image-or-video-bytes) arm. Router refuses binary models at submit today, so a queued result is JSON in practice; the binary arm is there for when that changes. The result is read within the same `maxBytes` cap as `run`'s (64 MiB by default, `null` to disable), and `get()` and `subscribe()` both take it per call. A result past the cap rejects with `response_too_large` and stays collectable, so another `get()` with a larger cap can still fetch it.
